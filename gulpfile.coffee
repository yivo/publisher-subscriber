...
gulp.task 'build', ->
  ...
  .pipe(replace(/bind__Base\(object, events\[i \- j\.\.\.i\], callback, context, once\)/g, 'event = events[i - j...i]; cb = (if once then onceWrap(object, event, callback, context) else callback);((object._ps ||= {})[fastProperty(event)] ||= []).push(undefined, cb, context)'))
  .pipe(replace(/bind__Base\(object, events, callback, context, once\)/g, 'cb = (if once then onceWrap(object, events, callback, context) else callback);((object._ps ||= {})[fastProperty(events)] ||= []).push(undefined, cb, context)'))
