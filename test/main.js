(function(QUnit) {
  ...
  QUnit.module('PublisherSubscriber');
  ...
  QUnit.test('stopListening cleans up references', function(assert) {
    ...
    assert.equal(_.size(a.__listening__), 0);
    assert.equal(_.size(b.__listeners__.event), 4);
  ...
  QUnit.test('listenToOnce with event maps cleans up references', function(assert) {
    ...
    b.trigger('one');
    assert.equal(_.size(a.__listening__), 1);
  });
  ...
  QUnit.test('bind a callback with a default context when none supplied', function(assert) {
    ...
    }, PublisherSubscriber.InstanceMembers);
  ...
  QUnit.test('bind with callback as property name', function(assert) {
    var obj = _.extend({}, PublisherSubscriber.InstanceMembers);
    var counter = 0;
  ...
  QUnit.test('listenTo with callback as property name', function(assert) {
    var obj = _.extend({}, PublisherSubscriber.InstanceMembers);
    var counter = 0;
    obj.fn = function() { counter++; };
